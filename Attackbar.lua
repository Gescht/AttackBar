...
function EnableBorder(nextBorder)
    local Border = "Border"
    ...
    if AttackBarDB.text == "thin" then
        DEFAULT_CHAT_FRAME:AddMessage("thin")
        ToggleBarBorder(Bordern, true)
        ToggleBarBorder(Border, false)
    elseif AttackBarDB.text == "none" then
        DEFAULT_CHAT_FRAME:AddMessage("none")
        ToggleBarBorder(Bordern, false)
        ToggleBarBorder(Border, false)
    else
        DEFAULT_CHAT_FRAME:AddMessage("standard")
        ToggleBarBorder(Bordern, false)
        ToggleBarBorder(Border, true)
    end
end
function ToggleBarBorder(border, toggle)
    DEFAULT_CHAT_FRAME:AddMessage("toggle "..tostring(toggle))
    if toggle then
        DEFAULT_CHAT_FRAME:AddMessage("show")
        for i, bar in ipairs(attackBars) do
            DEFAULT_CHAT_FRAME:AddMessage("show show")
            DEFAULT_CHAT_FRAME:AddMessage("show "..bar:GetName())
            getglobal(bar:GetName() .. border):Show()
        end
    else
        DEFAULT_CHAT_FRAME:AddMessage("hide")
        for i, bar in ipairs(attackBars) do
            DEFAULT_CHAT_FRAME:AddMessage("hide hide")
            DEFAULT_CHAT_FRAME:AddMessage("hide "..bar:GetName())
            getglobal(bar:GetName() .. border):Hide()
        end
    end
end
...
function Abar_selfhit(arg1)
    ...
    if not spell then
        a, b, spell = string.find(arg1, "Your (.+) crits")
    end
    if not spell then
        a, b, spell = string.find(arg1, "Your (.+) is")
    end
    if not spell then
        a, b, spell = string.find(arg1, "Your (.+) misses")
    end
    if spell then
        go = false
    end
...
function ebar_start(arg1)
    ...
    if not hitter then
        a, b, hitter = string.find(arg1, "(.+) crits you")
    end
    if not hitter then
        a, b, hitter = string.find(arg1, "(.+) misses you")
    end
    if not hitter then
        a, b, hitter = string.find(arg1, "(.+) attacks. You ")
    end
